##### modeling dinoNimi
rm(list=ls())#used to Remove Objects from a Specified Environmen
#setwd("~/Dropbox/Projekte/CEITER/Sea&Sense/SUSTAIN_Modeling")
setwd("C:/Users/Kasutaja/Desktop/DATA")
...
repeat{
  ...
  for(x in 1:length(listedFilesDinoNimi)){
    ...
    if((group.x==1)|(group.x==4)){
      ...
      agent.outcome <- agent_fx(par.x=par.x, data_student=data_student,labelQuery, structure_name, stimulus_structure=stimulus_structure, selfSelectedStimuli, ResponseSet,
                                m, V)
